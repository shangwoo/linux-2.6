...
static int ndesc_get_tx_status(void *data, struct asmmac_extra_stats *x,
...
static int ndesc_get_rx_status(void *data, struct asmmac_extra_stats *x,
			       struct dma_desc *p)
{
	int ret = good_frame;
	struct net_device_stats *stats = (struct net_device_stats *)data;
...
static int ndesc_get_rx_frame_len(struct dma_desc *p, int rx_coe_type)
{
	 ...
	 * engines. */
	if (rx_coe_type == ASMMAC_RX_COE_TYPE1)
		return p->des01.rx.frame_length - 2;
	else
		return p->des01.rx.frame_length;
}

const struct asmmac_desc_ops ndesc_ops = {
