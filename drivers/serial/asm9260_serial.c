...
struct asm9260_uart_port {
	...
	struct asm9260_dma_buffer	pdc_rx[2];	/* PDC receier */

	short			use_dma_tx;	/* enable PDC transmitter */
	struct asm9260_dma_buffer	pdc_tx;		/* PDC transmitter */
...
#ifdef CONFIG_SERIAL_ASM9260_DMA
static bool asm9260_use_dma_rx(struct uart_port *port)
{
	struct asm9260_uart_port *asm9260_port = to_asm9260_uart_port(port);

	return asm9260_port->use_dma_rx;
}

static bool asm9260_use_dma_tx(struct uart_port *port)
{
	struct asm9260_uart_port *asm9260_port = to_asm9260_uart_port(port);

	return asm9260_port->use_dma_tx;
}
#else
static bool asm9260_use_dma_rx(struct uart_port *port)
{
 ...
 */
static void asm9260_pdc_rxerr(struct uart_port *port, unsigned int status)
{
	/* clear error */
	...
	if (status & ASM9260_UART_PARITYERR)
		port->icount.parity++;
	if (status & ASM9260_UART_FRAMEERR)
		port->icount.frame++;
	if (status & ASM9260_UART_OVERRUNERR)
		port->icount.overrun++;
}
#endif
...
static void asm9260_rx_chars(struct uart_port *port)
{
	...
	while (!(status & ASM9260_UART_RXEMPTY)) {
		...
		if (unlikely(intr & (ASM9260_UART_PEIS | ASM9260_UART_FEIS
			...
			/* clear error */
			UART_PUT_STAT(port, 0);
			UART_PUT_INTR_CLR(port, ASM9260_UART_PEIS | ASM9260_UART_FEIS  | ASM9260_UART_OEIS | ASM9260_UART_BEIS);
 ...
 */
static void asm9260_tx_dma(struct uart_port *port)
{
   dbg("asm9260_tx_dma");
}
#endif
			      ...
			      struct ktermios *old)
{
	...
	*/
	...
	mode = UART_GET_LINECTRL(port) & ~(ASM9260_UART_PEN | ASM9260_UART_EPS | ASM9260_UART_STP2 | ASM9260_UART_FEN
					| ASM9260_UART_WLEN | ASM9260_UART_SPS | ASM9260_UART_BAUD_DIVFRA | ASM9260_UART_BAUD_DIVINT);

	baud = uart_get_baud_rate(port, termios, old, port->uartclk * 4 / UART_BAUD_DIV_MAX, port->uartclk / 16);
	bauddivint = (((port->uartclk << 2) / baud) & UART_BAUD_DIVINT_MASK) << 10;
	bauddivfrac = (((port->uartclk << 2) / baud) & UART_BAUD_DIVFRAC_MASK) << 8;
	/* byte size */
	...
	if (termios->c_iflag & IGNPAR)
		port->ignore_status_mask |= (ASM9260_UART_FEIS  | ASM9260_UART_PEIS);
...
static int asm9260_ioctl(struct uart_port *port, unsigned int cmd, unsigned long arg)
{
...
static unsigned int get_uart_clock(struct uart_port *port)
{
	unsigned int uart_clk_sel, syspll_clk, uart_src_clk, uart_clk;
	int uart_num;
				      ...
				      struct platform_device *pdev)
{
	...
	asm9260_port->use_dma_rx = data->use_dma_rx;
	asm9260_port->use_dma_tx = data->use_dma_tx;
					     ...
					     int *parity, int *bits)
{
	...
	mr = UART_GET_LINECTRL(port) & (ASM9260_UART_PEN | ASM9260_UART_EPS | ASM9260_UART_SPS);
...
static int __init asm9260_console_setup(struct console *co, char *options)
{
	...
	UART_PUT_CTRL2_SET(port, ASM9260_UART_TXE | ASM9260_UART_RXE | ASM9260_UART_ENABLE);
 ...
 */
static int __init asm9260_late_console_init(void)
{
	if (asm9260_default_console_device
	    && !(asm9260_console.flags & CON_ENABLED))
		register_console(&asm9260_console);

	return 0;
}

core_initcall(asm9260_late_console_init);
#endif
...
static int __devinit asm9260_serial_probe(struct platform_device *pdev)
{
	...
	if (!asm9260_use_dma_rx(&port->uart)) {
		ret = -ENOMEM;
